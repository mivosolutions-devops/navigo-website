...
const Feature = ({
  ...
  return (
    <div
      className={`w-full rounded-3xl border border-[#00BD5E] bg-gradient-to-br cursor-pointer flex transition-all duration-300 ease-out justify-start gap-3 p-3 backdrop-blur-xl overflow-hidden ${
        isActive
      ...
      onClick={onClick}
    >
      <div className='bg-gradient-to-b from-green-800 to-green-500 rounded-full min-w-14 max-sm:min-w-10 w-14 h-14 flex items-center justify-center max-sm:w-10 max-sm:h-10 transition-transform duration-300 ease-out'>
        <img
          src={icon}
          alt={title}
      ...
      <div className='flex flex-col items-start w-fit justify-center gap-2 transition-opacity duration-300 ease-out'>
        <h1
          className={`font-semibold w-full text-lg max-sm:text-sm transition-colors duration-300 ease-out ${
            isActive ? "text-[#00BD5E]" : "text-white"
          }`}
        >
          {title}
        </h1>
